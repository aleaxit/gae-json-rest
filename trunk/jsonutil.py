...
def path_to_classname_and_id(path):
  """ Get a (classname, id) pair from a path (id empty-string if absent from 
  the path) or (None, ''). """
...
def make_json_dict(instance):
  """ Make a dict suitable for send_json given an instance of db.Model sublass """
  result = id_of(x)
...
def parse_json_dict(cls, json_dict):
   """ Make a dict suitable for calling cls given a db.Model subclass and a 
   JSON-form dict."""
   result = dict()
...
def make_instance(cls, json_data):
  """ Make a db.Model subclass instance given the class object and a 
  JSON-form dict, return new JSON-form dict. """
  instance_data = parse_json_dict(cls, json_data)
  instance = cls(**instance_data)
  key = instance.put()
  json_data = make_json_dict( instance)
  json_data.update(id_of(instance))
  return json_data


def update_instance(instance, json_data):
  """ Update a db.Model subclass instance given class, instance, and a 
  JSON-form dict, return new JSON-form dict. """
