...
class TemplateTest(object):
  ...
  def run(self):
    ...
    modelnames = self.tester.request_and_show('GET', '/')
    try:
    ...
    doctorids = self.tester.request_and_show('GET', '/Doctor/')
    ...
    for i in range(0, num_doctors, 2):
      ...
      self.tester.silent_request('DELETE', '/Doctor/%s' % strid)
      deletions += 1
    self.emit('IDs of Doctors after some deletions:')
    doctorids = self.tester.silent_request('GET', '/Doctor/')
    self.emit(doctorids)
    ...
    post_result = self.tester.request_and_show('POST', '/Doctor/', post_body)
    ...
    new_doctor = self.tester.request_and_show('GET', new_doctor_path)
    ...
    put_result = self.tester.request_and_show('PUT', new_doctor_path, put_body)
    ...
    self.tester.request_and_show('PUT', new_doctor_path, put_body)
    ...
    a = int(self.tester.get_cookies().get('counter'))
    b = int(self.tester.get_cookies().get('counter'))
    c = int(self.tester.get_cookies().get('counter'))
    if a+1 != b or b+1 != c:
...
t = testutil.Tester(TemplateTest)
t.execute()
